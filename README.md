...
[btc_agentic_system]  ← 여러분이 업그레이드 (전략, 피처, 모델, 서치 루프)
        ...
        ▼
[btc_benchmark]  ← 동결된 심판 (데이터, 백테스터, 비용, walk-forward,
...
cd btc_benchmark && pip install -e ".[dev]" && pytest      # 심판 설치 + 검증
python -m scripts.bootstrap_data                            # 데이터 번들 재구축 (~10분)
cd ..
git clone https://github.com/YoonhoKim0527/btc_agentic_system.git
cd btc_agentic_system && pip install -e .
export BTC_BENCHMARK_REPO=../btc_benchmark                  # 데이터 번들이 있는 체크아웃
```
